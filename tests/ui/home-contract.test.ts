...
describe('branded home page contract', () => {
  ...
  it('drives personal copy, metrics, links and media from one project-owned config', () => {
  ...
  it('loads only the hero media eagerly and lazily loads below-fold media', () => {
  ...
  it('exposes resilient content and navigation entry links before scripts run', () => {
    expect(config).toMatch(/href:\s*"\/list\/"/)
    expect(config).toMatch(/href:\s*"\/knowledge\/"/)
    expect(index).not.toMatch(/client:(?:only|load)/)
    expect(index).not.toMatch(/document\.addEventListener/)
  })
})
