...
test.describe('content feature production contracts', () => {
  ...
  test('article assessment gives transient feedback and has no standalone route', async ({ page, request }) => {
    await page.goto('/posts/motor/algorithm/ALG-22-BLDC-Six-Step-Commutation/')
    ...
    await dialog.locator('[data-assessment-explanation]').first().evaluate(element => { (element as HTMLDetailsElement).open = true })
    await expect(dialog.locator('.assessment-explanation .katex').first()).toBeVisible()
    ...
    expect((await request.get('/posts/motor/algorithm/ALG-22-assessment/')).status()).toBe(404)
  })
})
