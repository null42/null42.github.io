...
tags:
  - imported
source: motor
sourcePath: learning-workspace/reference/formula-quick-ref.html
status: learning
visibility: public
summary: Imported from learning-workspace/reference/formula-quick-ref.html
chapterOrder: 5
navGroup: 入门与索引
navGroupOrder: 10
---
...
Motor Control Formula Quick Reference — 10大专题，核心公式一卡掌握

## 目录

1. [坐标变换](#1-坐标变换-coordinate-transforms)
2. [PMSM 电压方程](#2-pmsm-电压方程-pmsm-voltage-equations)
3. [转矩方程](#3-转矩方程-torque-equations)
4. [PI 参数设计](#4-pi-参数设计-pi-parameter-design)
5. [SVPWM 空间矢量调制](#5-svpwm-空间矢量调制)
6. [观测器](#6-观测器-observers)
7. [功率变换](#7-功率变换-power-conversion)
8. [热设计](#8-热设计-thermal-design)
9. [母线电容设计](#9-母线电容设计-bus-capacitor-design)
10. [通信](#10-通信-communication)

## 1. 坐标变换 Coordinate Transforms
...
等幅值变换系数为 2/3；等功率变换系数为 √(2/3)。此处为等幅值形式，假设 ia+ib+ic=0。
...
d轴对准转子磁链方向，q轴超前90°电角度。Park变换将交流量变为直流量。
...
## 2. PMSM 电压方程 PMSM Voltage Equations
...
第三项含反电动势 ωeψf 与交叉耦合项 ωeLdid，高速时反电动势是弱磁的根本原因。
...
基于零极点对消法：PI零点 1/τ=Kp/Ki=Ls/Rs 对消电气极点。ωc 通常取开关频率的 1/10~1/5。
...
速度环带宽通常为电流环带宽的 1/5~1/10。Ki 按经验取 Kp·ωs/10，确保低频增益足够。
...
增量式实现可避免积分累积问题：Δu[k] = Kp(e[k]-e[k-1]) + KiTse[k]。
...
SVPWM线性调制范围为 Vdc/√3，比SPWM的 Vdc/2 高 15.47%。过调制区可达 Vdc/√3 ~ 2Vdc/π。
...
小角度时 sin(Δθ) ≈ Δθ，PLL等效为II型伺服系统。Kp、Ki 决定跟踪带宽与动态性能。
...
降压变换器，CCM下输出电压与占空比成正比。D≤1 故 Vo ≤ Vin。
...
升压变换器，D→1 时 Vo→∞（理论值），实际受寄生参数限制。D不宜超过0.85。
...
PF = cosφ 仅在纯正弦波下成立。含谐波时 PF = cosφ · 1/√(1+THD²)。

## 8. 热设计 Thermal Design
...
必须保证 Tj < Tj,max（IGBT通常150°C，SiC MOSFET可达175°C）。散热器选型核心是降低 Rθ,sa。
...
此为经验公式，适用于单相整流后电容估算。三相PFC后纹波频率为2×电网频率，纹波更小。实际还需考虑 ESR 与纹波电流额定值。
