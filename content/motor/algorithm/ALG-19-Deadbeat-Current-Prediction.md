...
> ⚠️ **近似标注**：上式 $E_d = -\omega_e \psi_f T_s A_{d0} / L_s$ 为一阶近似表达式。精确 ZOH 离散化结果为 $E_d = -(1 - A_{d0}) \cdot \omega_e \psi_f / R_s$（其中 $A_{d0} = e^{-R_s T_s / L_s}$）。该近似在 $R_s T_s / L_s \ll 1$ 时成立（高 PWM 频率下通常满足，例如 $T_s \leq 100\mu s$、$R_s/L_s \leq 500$ s⁻¹ 时误差 < 1%）。当 $R_s T_s / L_s$ 不满足小量条件时（如低速大电阻电机），应使用精确公式。
