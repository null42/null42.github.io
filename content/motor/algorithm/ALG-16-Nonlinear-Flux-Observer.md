...
> ⚠️ **适用范围**：以下实现基于 SPMSM 假设（Ld = Lq），转子磁链提取直接使用 `psi - Ld·i`。IPMSM 的正确做法见本节末尾说明。
...
> ⚠️ **适用范围**：同上，以下实现基于 SPMSM 假设（Ld = Lq）。
...
> ⚠️ **注意**：上述 `psi_f_α = psi_α - Ld·i_α` 行仍沿用 SPMSM 简化公式，仅当 Ld = Lq 时严格成立。对于 Ld ≠ Lq 的 IPMSM，正确的转子磁链提取方法为：先将观测到的总磁链 `psi_α, psi_β` 和电流 `i_α, i_β` 变换到 dq 坐标系 → 计算 `psi_r_d = psi_d - Ld·i_d`、`psi_r_q = psi_q - Lq·i_q` → 再反旋转变换回 αβ 得到 `psi_f_α, psi_f_β`。
