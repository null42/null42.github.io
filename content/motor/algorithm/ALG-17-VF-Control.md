...
$$\frac{R_s I}{V}\bigg|_{f=f_n} \approx 2\% \sim 5\%, \quad \frac{R_s I}{V}\bigg|_{f=5Hz} \approx 20\% \sim 40\%$$
...
| 电流反馈提升 | $V_{boost} = R_s \cdot |I|$ | 最精确，需电流采样 |
...
2. **切换阶段（$f \approx f_{switch}$）：**
   ...
   - 当角度误差 $|\hat{\theta} - \theta_{ol}| < \theta_{threshold}$ 且持续 $N$ 个周期
   - 无扰动切换：$\theta_{ctrl} = \hat{\theta}$
...
| 关联点 | V/F弱磁 | FOC弱磁 |
|--------|---------|---------|
| 触发条件 | $f > f_{rated}$，电压饱和 | $|u_{dq}| > u_{max}$ |
