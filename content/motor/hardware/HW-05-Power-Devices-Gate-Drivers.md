...
> **⚠️ 驱动器与器件兼容性说明**：IR2130、DRV8301 等通用三相驱动器主要适用于 **Si MOSFET**（驱动电压10-15V、典型开关速度）。**不适用于 GaN HEMT**（驱动电压5-6V过高会击穿栅极）和 **SiC MOSFET**（18V推荐驱动电压不足、dV/dt耐受能力通常不够）。SiC需专用隔离驱动器（如UCC217xx），GaN需专用GaN驱动器（如LMG1020）。
...
> **⚠️ 实际SW电压不等于理想0V**：需考虑低侧MOSFET导通压降 V_DS(on) ≈ I_load × Rds(on) + PCB走线压降，大电流时SW可能达0.5~2V。设计自举电容的充电电压裕度时需扣除该压降，确保 V_CB = VCC - VF - V_DS(on) 仍满足高侧栅极驱动需求。
