...
>  **工程警告**：DCM 下 Buck 的电压增益不再是简单的 $$V_{out}=D \cdot V_{in}$$！DCM 增益为 $$V_{out} = \frac{2V_{in}}{1+\sqrt{1+4K/D^2}}$$，其中 $$K=2Lf_s/R_{load}$$。这会导致轻载时输出电压偏高，若不考虑可能触发过压保护！
...
>  注意：Boost 的输入电流等于电感平均电流，而输出电流是二极管平均电流。$$I_{in} = I_{out}/(1-D)$$
