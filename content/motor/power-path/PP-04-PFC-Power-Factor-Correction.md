...
> **⚠️ CCM模式公式适用条件**：以上电感电流纹波公式（ΔIL = Vout·D·(1-D)/(fs·L) 及其推导形式）在电感电流始终连续（i_L(t) > 0 恒成立）时精确成立。在CCM/DCM边界附近（轻载工况），实际纹波可能因部分开关周期进入DCM而偏离此计算值。设计时通常**按满载CCM条件计算L值**，再验证轻载时是否仍在CCM范围内（即最低负载电流 > ΔIL/2）。若轻载进入DCM，电感电流纹波将增大且控制特性变化，需评估是否可接受。
