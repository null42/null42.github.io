...
<span class="line"><span>  典型值：±0.05°~±1°</span></span></code></pre></div><p><strong>🔗 算法关联</strong>：角度精度直接决定Park变换精度 <strong>Park变换中的角度误差影响</strong>：</p><blockquote><ul><li>Id_actual = Id_cmd × cos(Δθ) + Iq_cmd × sin(Δθ)</li><li>Iq_actual = -Id_cmd × sin(Δθ) + Iq_cmd × cos(Δθ)</li></ul><p><strong>当Δθ很小时</strong>：</p><ul><li>Id_actual ≈ Id_cmd + Iq_cmd × Δθ</li><li>Iq_actual ≈ Iq_cmd - Id_cmd × Δθ</li></ul><p><strong>交叉耦合量</strong> = Iq × Δθ (对Id) 或 Id × Δθ (对Iq)</p><p><strong>示例</strong>：</p><ul><li><p>Δθ = 1° = 0.0175 rad</p></li><li><p>Iq = 10A, Id = 0</p></li><li><p>Id耦合 = 10 × 0.0175 = 0.175A (1.75%)</p></li><li><p>Iq损失 = 0 × 0.0175 = 0A</p></li><li><p>Δθ = 5° = 0.0873 rad</p></li><li><p>Id耦合 = 10 × 0.0873 = 0.873A (8.73%!) → 不可接受</p></li></ul><p><strong>对控制的影响</strong>：</p><ul><li>1°角度误差 → 1.7% Id/Iq交叉耦合 → 转矩脉动</li><li>5°角度误差 → 8.7% Id/Iq交叉耦合 → 电流环不稳定</li></ul><p><strong>精度要求</strong>：</p><ul><li>高性能伺服：角度误差 &lt; 0.1° → 交叉耦合 &lt; 0.17%</li><li>一般工业：角度误差 &lt; 1° → 交叉耦合 &lt; 1.7%</li><li>低成本应用：角度误差 &lt; 5° → 交叉耦合 &lt; 8.7%</li></ul></blockquote><h3 id="_5-2-角度延迟-→-观测器设计" tabindex="-1">5.2 角度延迟 → 观测器设计 <a class="header-anchor" href="#_5-2-角度延迟-→-观测器设计" aria-label="Permalink to &quot;5.2 角度延迟 → 观测器设计&quot;">​</a></h3><p><strong>硬件特性</strong>：</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>角度延迟来源：</span></span>
...
<span class="line"><span>  总延迟：10~700μs</span></span></code></pre></div><p><strong>🔗 算法关联</strong>：角度延迟需要观测器补偿</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>延迟导致的角度偏差：</span></span>
...
<span class="line"><span>  20位绝对式 → 1048576 步/转 → 0.00034°</span></span></code></pre></div><p><strong>🔗 算法关联</strong>：角度分辨率决定低速控制性能</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>低速时角度量化台阶的影响：</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>  结果:偏差减小到±5° → 基本可用 ⚠️</span></span>
<span class="line"><span></span></span>
...
<span class="line"><span>  1 LSB = 0.022° → n_min = 3.7rpm → 仍不够 ⚠️</span></span>
<span class="line"><span></span></span>
